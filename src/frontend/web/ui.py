...
def diffusionmagic_web_ui(generate: Generate, tpu_mode: bool = False) -> gr.Blocks:
    ...
    with gr.Blocks(
        ...
        gr.HTML("<center><H3>DiffusionMagic 3.0</H3></center>")
        if tpu_mode:
            with gr.Tabs():
                with gr.TabItem("Text to Image (TPU)"):
                    get_text_to_image_ui(generate.diffusion_text_to_image_flax)
                ...
                if stable_diffusion_type == StableDiffusionType.base:
                    ...
                    with gr.TabItem("Image Variations SDXL"):
                        get_image_variations_xl_ui(
                            generate.diffusion_image_variations_xl
                        )
                elif stable_diffusion_type == StableDiffusionType.inpainting:
                    get_image_variations_xl_ui
                with gr.TabItem("Settings"):
